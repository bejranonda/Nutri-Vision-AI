...
- **Admin** (`/[locale]/admin`, restricted) — operator console: quick-stat overview, user management (toggle admin, flip subscription tier), promo-code CRUD, a health-check view, and a **`/admin/scans` observability feed** (Round 15 — metadata-only: truncated userIds + item counts + `errorClass`, no emails/food-names/images, since photos are never stored). All four `/api/admin/*` mutation routes are rate-limited 30/min per IP (Round 14). Gated server-side by the `isAdmin` column on `users`; non-admins are silently redirected to `/login`. See [`docs/ADMIN_BOOTSTRAP.md`](docs/ADMIN_BOOTSTRAP.md) for how to create the first admin via `wrangler` (the schema ships with zero admins — no accidental admin via public sign-up).

> **First-impression UX + audit posture:** Rounds 7 → 16 (May–June 2026) iterated on the product through five lenses — fresh-user reading, live Playwright against the deploy, authed-surface walkthrough with a session cookie, a Web-Vitals + accessibility inventory, and an end-to-end full-user-journey spec. The biggest deltas:
...
> - **Round 13** found CI had **never passed** since it shipped in Round 11 (lockfile path + pydantic pin bugs) — fixed both for the repo's first green CI run; lesson: a gate nobody watches is not a gate.
> - **Round 14** burned down the security/resilience backlog: admin-route throttling, a real chat Gemini cascade walk, CSP in `Report-Only`, FK indexes (migration 0004). Unit tests **171 → 193**.
> - **Round 15** shipped `/admin/scans` (metadata-only observability — no PII, no stored photos) and gave the dead `errorClass` column writers so AI-pipeline regressions are visible without CF log access; handled timeouts now log `warn`, not `error`.
> - **Round 16** retargeted the discovery surface (SEO `<title>`/description/keywords, social cards, package + repo topic tags, README hero) from brand vocabulary to **search intent** — "AI food scanner", "blood sugar", "what to eat first". Pure metadata; **v2.1.14**.
